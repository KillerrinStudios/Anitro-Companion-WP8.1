...
namespace Anitro_Lockscreen
{
    public partial class App : Application
    {
        ...
        private void Application_Launching(object sender, LaunchingEventArgs e)
        {
            ...
            #region Background Task Info
            try
            {
                Debug.WriteLine("Looking for Task");
                string taskName = "AnitroLockScreenChanger";
                PeriodicTask oldTask = ScheduledActionService.Find(taskName) as PeriodicTask;

                if (oldTask != null) {
                    Debug.WriteLine("Task Found. Removing");
                    ScheduledActionService.Remove(taskName);
                }

                Debug.WriteLine("Making new Task");
                PeriodicTask task = new PeriodicTask(taskName);
                task.Description = "Changes lockscreen wallpaper";

                Debug.WriteLine("Installing new Task");
                ScheduledActionService.Add(task);

                if (Debugger.IsAttached)
                {
                    Debug.WriteLine("Launching Task for Test");
